...
routerAdd(
  "GET",
  "/custom_property_one_shop_bills",
  (c) => {
    try {
...
ON te.id = sh.tenant
WHERE sh.is_vacant = false AND sh.id = {:shop_id}
ORDER BY sh."order";
      `
        )
        ?.bind({
      ...
      return c.json(500, {
        error: ` Error getting custom_property_one_shop_bills : ${e.message}`,
      });
    }
  } /* optional middlewares */
);

// http://127.0.0.1:8090/custom_property_one_shop_bills?shop_id='5zfcguri77qp5i2'&curr_year=2023&prev_year=2023&curr_month=10&prev_month=10
//  parametes requered
